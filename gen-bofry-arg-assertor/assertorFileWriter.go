...
`
	...
	ARGV_DECIMAL_ASSERTION_TEMPLATE = `
func (assertor *{{.TypeName}}) {{.Name}}(validators ...arg.DecimalValidator) error {
	return arg.Decimals.Assert(assertor.argv.{{.Name}}, {{printf "%q" .Tag}},
		validators...,
	)
}
`

	ARGV_DECIMAL_PTR_ASSERTION_TEMPLATE = `
func (assertor *{{.TypeName}}) {{.Name}}(validators ...arg.DecimalPtrValidator) error {
	return arg.DecimalPtr.Assert(assertor.argv.{{.Name}}, {{printf "%q" .Tag}},
		validators...,
	)
}
`
...
var (
	FieldAssertionMap map[string]string = map[string]string{
		...
		DECIMAL_ASSERTION_TYPE:     ARGV_DECIMAL_ASSERTION_TEMPLATE,
		DECIMAL_PTR_ASSERTION_TYPE: ARGV_DECIMAL_PTR_ASSERTION_TEMPLATE,
